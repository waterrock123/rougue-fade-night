...
func _roll_relic_count() -> int:
	...
	var from_count = min(min_relic_count, max_relic_count)
	var to_count = max(min_relic_count, max_relic_count)
	return max(randi_range(from_count, to_count), 0)
...
func _get_candidate_relics(run_stats: RunStats, own_relic: Relic) -> Array[Relic]:
	...
	for relic in run_stats.shop.shopkeeper.relics:
		if relic == null:
			continue
		if exact_own_level and relic.level != own_relic.level:
			continue
		if not exact_own_level and relic.level > own_relic.level:
			continue
		result.append(relic)
	...
	if result.is_empty() and exact_own_level:
		for relic in run_stats.shop.shopkeeper.relics:
			if relic != null and relic.level <= own_relic.level:
				result.append(relic)
